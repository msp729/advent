...
day6 grid = do
    let
        ...
        possibleObstacles = tail guardPath
        ...
        loopObstacles =
            [ ob
            | ob <- possibleObstacles
            , let pre = getPrefix inGrid (prospect ob)
            , fst pre
            ]
